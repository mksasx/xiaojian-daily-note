...
桌面版与网页版的数据仍只保存在当前设备上，多设备之间通过 JSON 备份的“导出 / 导入并合并”传递数据。微信小程序默认本地优先，并在进入小程序、恢复到前台、网络重连或本地数据变化后，通过 CloudBase 按小程序 OpenID 自动隔离和合并同一微信账号的数据。
