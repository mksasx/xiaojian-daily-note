(function exposeTaskUtils(root, factory) {
  const api = factory();
  if (typeof module === 'object' && module.exports) module.exports = api;
  if (root) root.TaskUtils = api;
})(typeof window !== 'undefined' ? window : globalThis, function createTaskUtils() {
  ...
  function progressForDate(tasks, key) {
    ...
    return { total: dayTasks.length, completed, percent: dayTasks.length ? Math.round(completed / dayTasks.length * 100) : 0 };
  }
  ...
  function mergeStores(localInput, incomingInput, fallbackSettings = {}) {
    ...
    return {
      version: 2,
      tasks,
      deletedTasks: [...deletionMap.values()],
      settings: local.settings
    };
  }

  return { localDateKey, dateFromKey, shiftDateKey, tasksForDate, progressForDate, groupedHistory, normalizeBackup, mergeStores };
});
